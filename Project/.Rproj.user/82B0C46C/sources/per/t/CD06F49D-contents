...
# NOTE: Given Bradley's example in the above link actually works as intended,
#       I think this function works best (if not only) with ts (vs. zoo) objects
#       subject to the specific format of his series in the example. This can 
#       be achieved with monthly or annual data, but I've had a ton of trouble 
#       even getting the auto.arima() model to finish running after converting
#       the series to a ts() object, where as the model runs quickly if the 
#       series is a zoo() object. Relatedly, ts() is trash with daily series, 
#       however zoo() is much better.
#
# Conclusion: Try to only use this function with MONTHLY or ANNUAL series.
...
# Name:        plot_fx.R
#
# Title:       Custom plotting function for forecast data objects
#
# Version:     1.0
# Date:        2017-Oct-06
# Author:      Bradley Horn
# License:     GPL (>=2)
#
# Description: Custom plot function displays model training, fitted and forecast data
#              as lines and predictive intervals as ribbons
#
...
# Dev.Notes:   NA
#
# Depends:     The following R package libraries are required:
#              dplyr, ggplot, RColorBrewer, scales, forecast and zoo
#
# References:  NA
#
##-------------------------------------------------------------------------------------------##
